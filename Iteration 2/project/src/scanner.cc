...
namespace scanner {
  ...
  Token* Scanner::Token_Generator(const char *text) {
    ...
    regex_t *bool_keyword = make_regex("^(bool)");
    regex_t *true_keyword = make_regex("^(true)");
    regex_t *false_keyword = make_regex("^(false)");
    ...
    regex_t *while_keyword = make_regex("^(while)");
    ...
    struct make_str_regex_token_set str_regex_token_array[45]  = {
      {"^(I|i)nt", int_keyword, kIntKwd},
      {"^(F|f)loat", float_keyword, kFloatKwd},
      {"^(bool)", bool_keyword, kBoolKwd},
      {"^(true)", true_keyword, kTrueKwd},
      {"^(false)", false_keyword, kFalseKwd},
