 ...
 * Member Functions
 ******************************************************************************/
// March 3: lexeme, terminal, next -> lexeme_, terminal_, next_
ExtToken *ExtToken::ExtendToken(parser::Parser *p, Token *tokens) {
  switch (tokens->get_terminal_()) {
    case kLetKwd:
    ...
    case kNotEquals:
      return new RelationalOpToken(p, tokens, tokens->get_lexeme_());
    ...
    default:
      printf("%i not implemented extend", tokens->get_terminal_());
      fflush(stdout);
      assert(0);
  } /* switch() */
